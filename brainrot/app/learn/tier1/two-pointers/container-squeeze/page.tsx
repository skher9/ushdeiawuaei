...
function getMaxCompleted(): number {
  ...
  return parseInt(localStorage.getItem("pd_container-squeeze_levels") ?? "0", 10);
}

function getLevelStars(level: number): StarCount {
  if (typeof window === "undefined") return 0;
  return Math.min(3, Math.max(0, parseInt(localStorage.getItem(`pd_container-squeeze_level_${level}_stars`) ?? "0", 10))) as StarCount;
}
...
export default function ContainerSqueezeSelectPage() {
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "28px 20px 140px" }}>
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: DOCK.textDim, maxWidth: 400, margin: "0 auto" }}>
            Two walls, trapped water. Move the shorter wall. Find the maximum capacity.
          </p>
        </motion.div>
          ...
          {SQUEEZE_LEVELS.map((lvl, i) => {
            ...
            return (
              <LevelCard
                ...
                onPlay={() => router.push(`/learn/tier1/two-pointers/container-squeeze/${n}`)}
              />
            );
          })}
        </div>
        ...
        {maxCompleted >= 8 && (
          <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: DOCK.textDim }}>
              Maximum water trapped. Container Squeeze complete.
            </p>
          </motion.div>
        )}
      </div>
