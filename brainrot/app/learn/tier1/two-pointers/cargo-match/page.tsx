...
function getMaxCompleted(): number {
  ...
  return parseInt(localStorage.getItem("pd_cargo-match_levels") ?? "0", 10);
}

function getLevelStars(level: number): StarCount {
  if (typeof window === "undefined") return 0;
  return Math.min(3, Math.max(0, parseInt(localStorage.getItem(`pd_cargo-match_level_${level}_stars`) ?? "0", 10))) as StarCount;
}
...
export default function CargoMatchSelectPage() {
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "28px 20px 140px" }}>
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.3em", color: DOCK.cyan, marginBottom: 8 }}>
            ◆ ENTRY LEVEL · CLASSIC TWO POINTERS
          </div>
          ...
          <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: DOCK.textDim, maxWidth: 400, margin: "0 auto" }}>
            Two pointers converge from both ends. Find pairs that sum to the target.
          </p>
        </motion.div>
          ...
          {CARGO_LEVELS.map((lvl, i) => {
            ...
            return (
              <LevelCard
                ...
                onPlay={() => router.push(`/learn/tier1/two-pointers/cargo-match/${n}`)}
              />
            );
          })}
        </div>
        ...
        {maxCompleted >= 8 && (
          <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: DOCK.textDim }}>
              All 8 cargo manifests matched. Two pointers master.
            </p>
          </motion.div>
        )}
      </div>
