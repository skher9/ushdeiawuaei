...
import { WITNESS_LEVELS } from "@/components/games/tier1/binary-search/SearchCity/witness/levels";

function getMaxCompleted(): number {
  if (typeof window === "undefined") return 0;
  return parseInt(localStorage.getItem("sc_witness_levels") ?? "0", 10);
}

function getLevelStars(level: number): StarCount {
  if (typeof window === "undefined") return 0;
  const v = parseInt(localStorage.getItem(`sc_witness_level_${level}_stars`) ?? "0", 10);
...
export default function WitnessLevelSelectPage() {
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "28px 20px 140px" }}>
        ...
        >
          ...
          <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: NOIR.textDim, maxWidth: 400, margin: "0 auto" }}>
            No array. Binary search the answer space itself — feasibility checks replace comparisons.
          </p>
        </motion.div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
          {WITNESS_LEVELS.map((lvl, i) => {
            const n = i + 1;
            ...
            return (
              <LevelCard
                ...
                completed={completed}
                playable={playable}
                onPlay={() => router.push(`/learn/tier1/binary-search/witness/${n}`)}
              />
            );
          })}
        </div>
        ...
        {maxCompleted >= 8 && (
          ...
          >
            ...
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: NOIR.textDim }}>
              The witness spoke. Binary search on answer space — now your universal weapon.
            </p>
          </motion.div>
        )}
      </div>
