...
import { SMUGGLER_LEVELS } from "@/components/games/tier1/binary-search/SearchCity/smuggler/levels";

/* ── Progress helpers ────────────────────────────────────── */
function getMaxCompleted(): number {
  if (typeof window === "undefined") return 0;
  return parseInt(localStorage.getItem("sc_smuggler_levels") ?? "0", 10);
}

function getLevelStars(level: number): StarCount {
  if (typeof window === "undefined") return 0;
  const v = parseInt(localStorage.getItem(`sc_smuggler_level_${level}_stars`) ?? "0", 10);
    ...
    <motion.div
      ...
      style={{
        ...
        border: isCurrent
          ...
          : `1px solid ${completed ? "rgba(42,157,143,0.3)" : NOIR.border}`,
...
/* ── Page ────────────────────────────────────────────────── */
export default function SmugglerLevelSelectPage() {
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "28px 20px 140px" }}>
        ...
        >
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.3em", color: NOIR.amber, marginBottom: 8 }}>
            ◆ ENTRY LEVEL · CLASSIC BINARY SEARCH
          </div>
          ...
          <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: NOIR.textDim, maxWidth: 400, margin: "0 auto" }}>
            A smuggler hides in one of N warehouses. Every guess cuts the remaining possibilities in half.
          </p>
        </motion.div>
          ...
          {SMUGGLER_LEVELS.map((lvl, i) => {
            ...
            return (
              <LevelCard
                ...
                completed={completed}
                playable={playable}
                onPlay={() => router.push(`/learn/tier1/binary-search/smuggler/${n}`)}
              />
            );
          })}
        </div>
        ...
        {maxCompleted >= 8 && (
          ...
          >
            ...
            <p style={{ fontFamily: "var(--font-tac)", fontSize: 13, color: NOIR.textDim }}>
              All 8 warehouses searched. The smuggler never stood a chance.
            </p>
          </motion.div>
        )}
      </div>
